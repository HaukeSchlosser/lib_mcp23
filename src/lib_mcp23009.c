...
static int set_iocon_for_interrupt(mcp23009_t *dev) {
    ...
    if (!dev) {
        fprintf(stderr, "[mcp23009::set_iocon_for_interrupt] ERROR: Invalid device handle\n");
        return -1;
    }
    ...
    return mcp23009_write(dev, MCP_009_IOCON, iocon);
}
...
static int set_pins_input_and_pullups(mcp23009_t *dev, uint8_t bitmask) {
    ...
    int16_t r = mcp23009_read(dev, MCP_009_IODIR);
    if (r < 0) {
    ...
    if (mcp23009_write(dev, MCP_009_GPPU, gppu) < 0) {
        return -1;
    }
...
static int configure_interrupt_mode(mcp23009_t *dev, uint8_t bitmask, uint8_t interrupt_mode) {
    ...
    int16_t r = mcp23009_read(dev, MCP_009_INTCON);
    if (r < 0) {
    ...
    if (interrupt_mode == MCP_009_CHANGE_ANY) {
        intcon &= ~bitmask;
        if (mcp23009_write(dev, MCP_009_INTCON, intcon) < 0) {
        ...
        r = mcp23009_read(dev, MCP_009_DEFVAL);
        if (r < 0) {
        ...
        if (mcp23009_write(dev, MCP_009_DEFVAL, defval) < 0) {
...
static int update_gpinten(mcp23009_t *dev, uint8_t bitmask, uint8_t enable) {
    ...
    int16_t r = mcp23009_read(dev, MCP_009_GPINTEN);
    if (r < 0) {
    ...
    if (enable == MCP_009_INT_ENABLE) {
        gpinten |= bitmask;
    } else {
        gpinten &= (uint8_t)~bitmask;
    }

    return mcp23009_write(dev, MCP_009_GPINTEN, gpinten);
}
...
static int clear_pending_int(mcp23009_t *dev) {
    ...
    return mcp23009_read(dev, MCP_009_INTCAP);
}
...
int mcp23009_init(mcp23009_t *dev, uint8_t bus, uint32_t speed_hz, uint8_t address) {
    ...
    if (!dev) {
        fprintf(stderr, "[mcp23009_init] ERROR Invalid device handle\n");
        return -1;
    }

    if (address > MCP_009_ADDR_MAX) {
        fprintf(stderr, "[mcp23009_init] ERROR Invalid address input: 0x%02X\n", address);
        fprintf(stderr, "[mcp23009_init] ERROR Expected range: 0x00 - 0x%02X\n", MCP_009_ADDR_MAX);
        return -1;
    }

    if (speed_hz < MCP_009_SPEED_SLOW) {
        printf("[mcp23009_init] INFO: SPI speed too low, setting to minimum %u Hz\n", MCP_009_SPEED_SLOW);
        speed_hz = MCP_009_SPEED_SLOW;
    }
    if (speed_hz > MCP_009_SPEED_FAST) {
        printf("[mcp23009_init] INFO: SPI speed too high, setting to maximum %u Hz\n", MCP_009_SPEED_FAST);
        speed_hz = MCP_009_SPEED_FAST;
    }

    char bus_name[16];
    int n = snprintf(bus_name, sizeof bus_name, "/dev/i2c-%u", (unsigned)bus);
    if (n < 0 || n >= (int)sizeof bus_name) {
        fprintf(stderr, "[mcp23009_init] ERROR Constructing bus name\n");
        return -1;
    }

    if ((fd = open(bus_name, O_RDWR)) < 0) {
        fprintf(stderr, "[mcp23009_init] ERROR Could not open I2C device: %s\n", bus_name);
    ...
    if (ioctl(fd, I2C_SLAVE, i2c_addr) <0){
        fprintf(stderr, "[mcp23009_init] ERROR Could not select I2C slave 0x%02X on %s\n", 
            i2c_addr, bus_name);
        perror("I2C_SLAVE");
        close(fd);
        return -1;
...
int8_t mcp23009_write(mcp23009_t *dev, uint8_t reg, uint8_t data) {
    ...
    if (!dev) {
        fprintf(stderr, "[mcp23009_write] ERROR Invalid device handle\n");
        return -1;
    }

    if (dev->fd < 0) {
        fprintf(stderr, "[mcp23009_write] ERROR Invalid file descriptor\n");
        return -1;
    }

    if (reg > MCP_009_REG_MAX) {
        fprintf(stderr, "[mcp23009_write] ERROR Invalid register address: 0x%02X\n", reg);
        fprintf(stderr, "[mcp23009_write] ERROR Expected range: 0x00 - 0x%02X\n", MCP_009_REG_MAX);
        return -1;
    }
    ...
    if (written != 2) {
        fprintf(stderr, "[mcp23009_write] ERROR Writing to register 0x%02X\n", reg);
        perror(NULL);
        return -1;
    }
...
int8_t mcp23009_interrupt(mcp23009_t *dev, uint8_t enable, uint8_t bitmask, uint8_t interrupt_mode) {
    ...
    if (enable != MCP_009_INT_ENABLE && enable != MCP_009_INT_DISABLE) {
        ...
        fprintf(stderr, "[mcp23009_interrupt] ERROR Expected values: MCP_009_INT_ENABLE (%u) or MCP_009_INT_DISABLE (%u)\n",
            MCP_009_INT_ENABLE, MCP_009_INT_DISABLE);
        return -1;
    }

    if (interrupt_mode != MCP_009_CHANGE_ANY && interrupt_mode != MCP_009_COMPARE_DEFVAL) {
        ...
        fprintf(stderr, "[mcp23009_interrupt] ERROR Expected values: MCP_009_CHANGE_ANY (%u) or MCP_009_COMPARE_DEFVAL (%u)\n",
            MCP_009_CHANGE_ANY, MCP_009_COMPARE_DEFVAL);
        return -1;
    }
