...
int main() {
    ...
    if (mcp23009_interrupt(&dev, MCP_009_INT_ENABLE, bitmask, MCP_009_CHANGE_ANY) < 0) return 1;
    if (mcp23009_read(&dev, MCP_009_INTCAP) < 0) return 1;
    ...
    printf("[mcp23009_test_interrupt] Run test program...\n");
    printf("[mcp23009_test_interrupt] Press crtl + c to stop...\n");
    ...
    while(1) {
        ...
        int16_t inf = mcp23009_read(&dev, MCP_009_INTF);
        int16_t cap = mcp23009_read(&dev, MCP_009_INTCAP);
        if (inf >= 0 && cap >= 0) {
