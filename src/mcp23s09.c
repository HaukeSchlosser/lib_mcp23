 ...
 */
static mcp_err_t set_iocon_for_interrupt(mcp23s09_t *dev) {
...
static int set_pins_input_and_pullups(mcp23s09_t *dev, uint8_t bitmask) {
    ...
    int rc = mcp23s09_write(dev, MCP_IODIR, iodir);
    ...
    rc = mcp23s09_write(dev, MCP_GPPU, gppu);
    if (rc < 0) return (mcp_err_t)rc;
...
static int configure_interrupt_mode(mcp23s09_t *dev, uint8_t bitmask, uint8_t interrupt_mode) {
    ...
    if (interrupt_mode == MCP_CHANGE_ANY) {
        ...
        rc = mcp23s09_write(dev, MCP_INTCON, intcon);
        if (rc < 0) return (mcp_err_t)rc;
    } else {
        intcon |= bitmask;
        rc = mcp23s09_write(dev, MCP_INTCON, intcon);
        ...
        rc = mcp23s09_write(dev, MCP_DEFVAL, defval);
        if (rc < 0) return (mcp_err_t)rc;
    }
...
static int update_gpinten(mcp23s09_t *dev, uint8_t bitmask, uint8_t enable) {
    ...
    int rc = mcp23s09_write(dev, MCP_GPINTEN, gpinten);
    if (rc < 0) return (mcp_err_t)rc;
...
static int clear_pending_int(mcp23s09_t *dev) {
    ...
    int16_t r = mcp23s09_read(dev, MCP_INTCAP);
    if (r < 0) return (mcp_err_t)r;

    return MCP_OK;
}

mcp_err_t mcp23s09_init(mcp23s09_t *dev, uint8_t bus, uint8_t cs, uint8_t spi_mode, uint32_t speed_hz) {

    if (!dev) return MCP_EPARAM;
    if (bus >= 2)  return MCP_FAIL(dev, MCP_EPARAM);
    if (cs  >= 2)  return MCP_FAIL(dev, MCP_EPARAM);
    if (spi_mode != SPI_MODE_0 && spi_mode != SPI_MODE_3) return MCP_FAIL(dev, MCP_EPARAM);
    ...
    if (ioctl(fd, SPI_IOC_RD_MAX_SPEED_HZ, &aps) == 0 && aps) {
        if (aps != dev->speed_hz) {
            printf("[mcp23::mcp23s09_init] INFO: driver applied %u Hz (requested %u Hz)\n", aps, dev->speed_hz);
        }
        dev->speed_hz = aps;
    }
...
mcp_err_t mcp23s09_write(mcp23s09_t *dev, uint8_t reg, uint8_t data) {
...
int16_t mcp23s09_read(mcp23s09_t *dev, uint8_t reg) {
    ...
    uint8_t tx[3] = {MCP_09S_READ_CMD, reg, 0x00};
...
mcp_err_t mcp23s09_write_pin(mcp23s09_t *dev, uint8_t reg, uint8_t pin, uint8_t data) {
    ...
    int16_t r = mcp23s09_read(dev, reg);
    if (r < 0) return (mcp_err_t)r;
    ...
    mcp_err_t rc = mcp23s09_write(dev, reg, val);
    if (rc < 0) return rc;
...
mcp_err_t mcp23s09_interrupt(mcp23s09_t *dev, uint8_t enable, uint8_t bitmask, uint8_t interrupt_mode) {
...
mcp_err_t mcp23s09_led(mcp23s09_t *dev, uint8_t enable) {
    ...
    mcp_err_t rc = mcp23s09_write(dev, MCP_IODIR, 0xFF); // reset
    if (rc < 0) return rc;

    rc = mcp23s09_write(dev, MCP_IODIR, 0x00); // IODIR: all output
    if (rc < 0) return rc;

    rc = mcp23s09_write(dev, MCP_OLAT, 0xFF); // OLAT: all high (LEDs off)
    if (rc < 0) return rc;

    if (enable == MCP_LED_ENABLE) {
        rc = mcp23s09_write(dev, MCP_OLAT, 0x00);
        if (rc < 0) return rc;
        return MCP_OK;
    }
    if (enable == MCP_LED_DISABLE) {
        rc = mcp23s09_write(dev, MCP_OLAT, 0xFF);
        if (rc < 0) return rc;
        return MCP_OK;
    }
